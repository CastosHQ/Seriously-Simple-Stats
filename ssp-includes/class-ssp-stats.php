...
class SSP_Stats {
	...
	public function track_download ( $file = '', $episode = 0, $referrer = '' ) {
		...
		$ssp_url = "https://ipapi.co/" . $ip_address . "/json/";
		$ssp_loc = file_get_contents($ssp_url);
		
		$ssp_obj = json_decode($ssp_loc);
		...
		$insert_row = $wpdb->insert(
			...
			array(
				...
				'city' => $ssp_obj->city,
				'region' => $ssp_obj->region,
				'country' => $ssp_obj->country,
				'postal' => $ssp_obj->postal,
				'org' => $ssp_obj->org,
			),
			array(
				'%d',
...
$html .= '<div class="postbox">' . "\n";
	$html .= '<' . $metabox_title . ' class="hndle ui-sortable-handle">' . "\n";
		$html .= '<span>' . __( 'Weekly Listens Custom', 'seriously-simple-stats' ) . '</span>' . "\n";
	$html .= '</' . $metabox_title . '>' . "\n";
	$html .= '<div class="inside">' . "\n";
		...
		$html .= '<' . $metabox_title . ' class="hndle ui-sortable-handle">' . "\n";
			$html .= '<span>' . __( 'Weekly Listens Custom', 'seriously-simple-stats' ) . '</span>' . "\n";
			$html .= '</' . $metabox_title . '>' . "\n";
			$html .= '<div id="weekly_listens_custom"></div>' . "\n";
...
$html .= '</div>' . "\n";
					...
					$html .= '<div class="postbox" id="last-three-months-container">' . "\n";
						...
						$html .= '<div class="inside">' . "\n";
							...
							$total_per_page = apply_filters( 'ssp_stats_three_months_per_page', 50 ); 
					...
					$html .= '<div class="postbox" id="top-ten-container">' . "\n";
						$html .= '<' . $metabox_title . ' class="hndle ui-sortable-handle">' . "\n";
					    	$html .= '<span>' . __( 'Most Listened to Episodes of All Time', 'seriously-simple-stats' ) . '</span>' . "\n";
						$html .= '</' . $metabox_title . '>' . "\n";
						$html .= '<div class="inside">' . "\n";

							$sql = "SELECT COUNT(id) AS listens, post_id FROM $this->_table GROUP BY post_id ORDER BY listens DESC";
							$results = $wpdb->get_results( $sql );
	...
	public function chart_data ( ) {
		...
		$output .= $this->weekly_listens_chart_custom();
	...
	/**
	 * Return data for Weekly Listens chart Custom
	 * @access private
	 * @since  1.0.0
	 ...
	 */
	private function weekly_listens_chart_custom(){
		...
		$weekly_listens_array = array();
		$current_week_unix = $this->current_time;
		$last_week_unix = strtotime( '-1 week', $current_week_unix );
		$n=1;
		
		$result_table = "<table><tr><td>Week Pos</td><td>Listens</td></tr> \n";
		
		while($last_week_unix > 1493942399){
			
			$listens_last_week = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(id) FROM $this->_table WHERE date BETWEEN %d AND %d $episode_id_where", $last_week_unix, $current_week_unix ) );
			$weekly_listens_array[] = array($n, $listens_last_week);
			$result_table .= "<tr><td>$n</td><td>$listens_last_week</td></tr>";
			$current_week_unix = $last_week_unix;
			$last_week_unix = strtotime( '-1 week', $current_week_unix );
			$n++;

			}
		$data = array();
		$time_now2 = $this->current_time;
		foreach($weekly_listens_array as $week => $listens){
			
			//$week_number = "-" . $week . " weeks";
			//$current = strtotime( $week_number, $this->current_time);
			$time_now = strtotime( '-1 week', $time_now2);
			$time_now2 = $time_now;
			$make_date = date("M j", $time_now);
			//$data[] = array( $time_now, $listens[1]);
			array_unshift($data, array($make_date, $listens[1]));
			
		}

		return $this->generate_chart( 'LineChart', '', $columns, $data, 'weekly_listens_custom' );
